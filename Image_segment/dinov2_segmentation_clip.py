...
def run_dinov2_sam_segmentation(img_path, output_path, k_clusters=3):
    ...
    # ================= 1. 加载所有大模型 =================
    print("正在加载世界上最强大的三引擎流水线...")
    print(" 1/3 加载 DINOv2-Base (负责：无监督寻找物体)...")
    dinov2_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14')
    dinov2_model.eval().to(device)
    
    print(" 2/3 加载 SAM-ViT-H (负责：像素级完美边缘裁剪)...")
    sam = sam_model_registry["vit_h"](checkpoint="sam_vit_h_4b8939.pth")
    sam.to(device=device)
    sam_predictor = SamPredictor(sam)

    print(" 3/3 加载 CLIP 大语言视觉模型 (负责：给裁出来的东西起名字)...")
    try:
        classifier = pipeline(
        ...
        print(f"⚠️ 无法连接 HuggingFace 下载 CLIP 视觉字典，因为网络墙/缓存损坏。分类功能将进入降级模式。")
        classifier = None
    ...
    for cluster_id in range(k_clusters):
        ...
        for cnt in contours:
            if cv2.contourArea(cnt) > (w_orig * h_orig * 0.05):
                ...
                cv2.putText(result_img, display_label, (x_text, y_text - 10), 
